...
public class AgentPacmanCharacter : Agent
{
    ...
    public override void OnEpisodeBegin()
    {
        ...
        int w = _levelManager.levelData.LevelWidth;
        int h = _levelManager.levelData.LevelHeight;
        _maxDist = (float) Math.Sqrt(w * w + h * h);
    }
    ...
    public override void OnActionReceived(ActionBuffers actionBuffers)
    {
        ...
        if (type == Type.Ghost)
        {
            ...
            foreach (var ps in _levelManager.playerStates)
            {
                if (ps.agentScript.type == Type.Pacman)
                {
                    d = Vector3.Distance(transform.localPosition, ps.agentScript.gameObject.transform.localPosition);
                }
            }

            d /= _maxDist;
            d =  d * 2 - 1;
            // Add reward based on ghost proximity to Pac-Man
            if (_levelManager.levelData.powerPillActive)
            {
                SetReward(d * m_Existential);
            }
            else
            {
                SetReward(-d * m_Existential);
